...
public class AbstractPlayerInteraction {
	...
	/**
	 * Gives item with the specified id or takes it if the quantity is negative. Note that this does
	 * NOT take items from the equipped inventory. randomStats for generating random stats on the
	 * generated equip.
	 * 
	 * @param id
	 * @param quantity
	...
	public boolean gainItem(int id, short quantity, boolean randomStats, boolean show) {
		if (quantity >= 0) {
			...
			if (type.equals(MapleInventoryType.EQUIP) && !ii.isThrowingStar(item.getItemId())
				...
				if (!getPlayer().getInventory(type).isFull()) {
					...
					c.getPlayer().dropMessage(1, "Your inventory is full. Please remove an item from your "
							+ type.name().toLowerCase() + " inventory.");
					return false;
				}
			...
			MapleInventoryManipulator.removeById(c, MapleItemInformationProvider.getInstance().getInventoryType(id), id,
					-quantity, true, false);
		}
		return true;
	}
		...
		for (MapleCharacter chr : party) {
			...
			if (quantity >= 0) {
				...
				MapleInventoryManipulator.removeById(cl,
						MapleItemInformationProvider.getInstance().getInventoryType(id), id, -quantity, true, false);
			}
			cl.getSession().write(MaplePacketCreator.getShowItemGain(id, quantity, true));
		}
	}
	// PQ gain EXP: Multiplied by channel rate here to allow global values to be input direct into
	// NPCs
	...
	public void removeFromParty(int id, List<MapleCharacter> party) {
		for (MapleCharacter chr : party) {
			...
			if (possesed > 0) {
				MapleInventoryManipulator.removeById(c, MapleItemInformationProvider.getInstance().getInventoryType(id),
						id, possesed, true, false);
