...
public class ZakSquad {
	...
	public static int createSquad(int ch, int id) throws SQLException {
		...
		if (rs.getInt("status") == 0) {
			PreparedStatement ps1 = con
					.prepareStatement("UPDATE zaksquads SET leaderid = ?, status = 1, members = 1 WHERE channel = ?");
			ps1.setInt(1, id);
			ps1.setInt(2, ch);
			ps1.executeUpdate();
			rs.close();
	...
	public static int checkLeader(int ch, int id) throws SQLException {
		...
		PreparedStatement ps = con
				.prepareStatement("SELECT count(*) as num FROM zaksquads WHERE channel = ? AND leaderid = ?");
		ps.setInt(1, ch);
		ps.setInt(2, id);
		rs = ps.executeQuery();
		rs.next();
	...
	public static int removeSquad(int ch, int id) throws SQLException {
		...
		if (rs.getInt("num") > 0) {
			PreparedStatement ps1 = con
					.prepareStatement("UPDATE zaksquads SET leaderid = 0, status = 0, members = 0 WHERE channel = ?");
			ps1.setInt(1, ch);
			ps1.executeUpdate();
			rs.close();
			ps.close();
