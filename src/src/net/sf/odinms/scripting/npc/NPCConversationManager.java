...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	public void warpParty(int mapId, int exp, int meso) {
		for (MaplePartyCharacter chr_ : getPlayer().getParty().getMembers()) {
			...
			if ((curChar.getEventInstance() == null && c.getPlayer().getEventInstance() == null)
	...
	public void warpRandom(int mapid) {
		...
		MaplePortal portal = target.getPortal(rand.nextInt(target.getPortals().size())); // generate
																							// random
																							// portal
	...
	public String searchItem(String item) {
		...
		getPlayer().getMap().broadcastMessage(getPlayer(), MaplePacketCreator.showJobChange(getPlayer().getId()),
				false);
		for (Pair<Integer, String> itemPair : MapleItemInformationProvider.getInstance().getAllItems()) {
			if (itemPair.getRight().toLowerCase().contains(item.toLowerCase())) {
				message.append("\r\n#L" + itemPair.getLeft() + "##i" + itemPair.getLeft() + "# - #b"
						+ itemPair.getRight() + "#k#l");
			}
		}
	...
	public void broadcastMessage(int type, String message) {
		try {
			getPlayer().getClient().getChannelServer().getWorldInterface().broadcastMessage(null,
					MaplePacketCreator.serverNotice(type, message).getBytes());
	...
	public void setClan(int set) {
		...
		try {
			getPlayer().getClient().getChannelServer().getWorldInterface().broadcastToClan(
					(getPlayer().getName() + " has entered the clan ! Give him a nice welcome.").getBytes(), set);
		} catch (RemoteException e) {
			c.getChannelServer().reconnectWorld();
		}
