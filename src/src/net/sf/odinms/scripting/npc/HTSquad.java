...
public class HTSquad {
	...
	public static int createSquad(int ch, int id) throws SQLException {
		...
		if (results.getInt("status") == 0) {
			PreparedStatement ps1 = con
					.prepareStatement("UPDATE htsquads SET leaderid = ?, status = 1, members = 1 WHERE channel = ?");
			ps1.setInt(1, id);
			ps1.setInt(2, ch);
			ps1.executeUpdate();
			ps.close();
	...
	public static int checkLeader(int ch, int id) throws SQLException {
		...
		PreparedStatement ps = con
				.prepareStatement("SELECT count(*) as num FROM htsquads WHERE channel = ? AND leaderid = ?");
		ps.setInt(1, ch);
		ps.setInt(2, id);
		results = ps.executeQuery();
		results.next();
	...
	public static int removeSquad(int ch, int id) throws SQLException {
		...
		if (results.getInt("num") > 0) {
			PreparedStatement ps1 = con
					.prepareStatement("UPDATE htsquads SET leaderid = 0, status = 0, members = 0 WHERE channel = ?");
			ps1.setInt(1, ch);
			ps1.executeUpdate();
			ps.close();
			ps1.close();
