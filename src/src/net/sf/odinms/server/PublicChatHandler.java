...
public class PublicChatHandler {
	...
	private static void addPlayer(MapleCharacter chr) {
		...
		for (int chrIds : playerHolder.keySet()) {
			MapleCharacter chrs = ChannelServer.getInstance(playerHolder.get(chrIds)).getPlayerStorage()
			...
			chrs.getClient().getSession()
					.write(MaplePacketCreator.multiChat("", chr.getName() + " has been added to the chat.", 2));
		}
	}
	...
	private static void removePlayer(MapleCharacter chr) {
		if (playerHolder.containsKey(chr.getId())) {
			...
			for (int chrIds : playerHolder.keySet()) {
				...
				chrs.getClient().getSession()
						.write(MaplePacketCreator.multiChat("", chr.getName() + " has left the chat.", 2));
			}
		}
	}
