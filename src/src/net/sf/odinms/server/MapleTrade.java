...
public class MapleTrade {
	...
	public void lock() {
		locked = true;
		// chr.getClient().getSession().write(MaplePacketCreator.getTradeConfirmation()); // own
		// side shouldn't see other side whited
	...
	public void chat(String message) {
		...
		if (partner != null) {
			partner.getChr().getClient().getSession()
					.write(MaplePacketCreator.shopChat(chr.getName() + " : " + message, 1));
		}
	}
	...
	public static void completeTrade(MapleCharacter c) {
		...
		if (partner.isLocked()) {
			...
			if (!local.fitsInInventory() || !partner.fitsInInventory()) {
				cancelTrade(c);
				c.getClient().getSession().write(MaplePacketCreator.serverNotice(5,
						"There is not enough inventory space to complete the trade."));
				partner.getChr().getClient().getSession().write(MaplePacketCreator.serverNotice(5,
						"There is not enough inventory space to complete the trade."));
				return;
			}
			local.complete2();
		...
		if (trade != null) {
			if (trade.getPartner() != null) {
				...
				other.getClient().getSession()
						.write(MaplePacketCreator.serverNotice(5, c.getName() + " has declined your trade request"));
