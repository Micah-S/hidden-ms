...
public class PvPLibrary {
	...
	public static void monsterBomb(final MapleCharacter player, final MapleCharacter attackedPlayers,
			final AbstractDealDamageHandler.AttackInfo attack) {
		for (int dmgpacket = 0; dmgpacket < attack.numDamage; dmgpacket++) {
			TimerManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					player.getClient().getSession().write(MaplePacketCreator.damagePlayer(attack.numDamage,
							pvpMob.getId(), attackedPlayers.getId(), pvpDamage));
					...
					attackedDamage = 0;
					if (magicguard) {
						player.getClient().getSession().write(MaplePacketCreator.serverNotice(5,
								player.getName() + " has partially blocked your attack with magic guard!"));
						magicguard = false;
					}
					if (mesguard) {
						player.getClient().getSession().write(MaplePacketCreator.serverNotice(5,
								player.getName() + " has partially blocked your attack with mesoguard!"));
						mesguard = false;
					}
					if (magicrecovery) {
						attackedPlayers.getClient().getSession().write(MaplePacketCreator.serverNotice(5,
								player.getName() + " has partially absorbed your MP with MP Eater!"));
						magicrecovery = false;
					}
					// rewards
	...
	public static void playerReward(MapleCharacter player, MapleCharacter attackedPlayers) {
		...
		attackedPlayers.getMap().spawnMesoDrop(mesoReward, mesoReward, attackedPlayers.getPosition(), attackedPlayers,
	...
	public synchronized static void doPvP(MapleCharacter player, AbstractDealDamageHandler.AttackInfo attack) {
		...
		getDirection(attack);
		for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), PvPLibrary.maxDis,
				PvPLibrary.maxHeight, Collections.unmodifiableCollection(player.getMap().getCharacters()))) {
