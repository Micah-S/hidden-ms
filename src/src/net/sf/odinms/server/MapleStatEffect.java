...
public class MapleStatEffect implements Serializable {
	...
	private static org.slf4j.Logger				log					= org.slf4j.LoggerFactory
			.getLogger(MapleStatEffect.class);
	...
	private static void addBuffStatPairToListIfNotZero(List<Pair<MapleBuffStat, Integer>> list, MapleBuffStat buffstat,
			Integer val) {
		if (val.intValue() != 0) {
		...
		if (skill) {
			switch (sourceid) {
				...
				case 3221002: // sharp eyes marksmen
					...
					statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SHARP_EYES,
					...
					if (!mob.isBoss()) {
						...
						if (absorbMp > 0) {
							...
							applyto.getMap().broadcastMessage(applyto,
									MaplePacketCreator.showBuffeffect(applyto.getId(), sourceid, 1, (byte) 3), false);
						}
					}
					break;
			}
	...
	private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
		...
		if (isDispel() && makeChanceResult()) {
			MonsterStatus[] remove = { MonsterStatus.ACC, MonsterStatus.AVOID, MonsterStatus.WEAPON_IMMUNITY,
					MonsterStatus.MAGIC_IMMUNITY, MonsterStatus.SPEED };
			for (MapleMapObject _mob : applyfrom.getMap().getMapObjectsInRange(applyfrom.getPosition(),
					30000 + applyfrom.getSkillLevel(SkillFactory.getSkill(sourceid)) * 1000,
				...
				if (mob != null && mob.isAlive() && !mob.getMonsterBuffs().isEmpty()) {
					for (int i = 0; i < remove.length; i++) {
						if (mob.getMonsterBuffs().contains(remove[i])) {
							...
							MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(mob.getObjectId(),
		...
		if (isShadowClaw()) {
			...
			if (projectile == 0) {
				...
				MapleInventoryManipulator.removeById(applyto.getClient(), MapleInventoryType.USE, projectile, 200,
						false, true);
			}
		}
		if (overTime) {
	...
	private void applyBuff(MapleCharacter applyfrom) {
		if (isPartyBuff() && (applyfrom.getParty() != null || isGmBuff())) {
			...
			for (MapleMapObject affectedmo : affecteds) {
				...
				if (affected != null && applyfrom == null && isHeal() && affected != applyfrom
						&& affected.getParty() == applyfrom.getParty() && affected.isAlive()) {
					int expadd = (int) ((calcHPChange(applyfrom, true) / 10)
							* (applyfrom.getClient().getChannelServer().getExpRate() + ((Math.random() * 10) + 30))
							* (Math.floor(
									Math.random() * (applyfrom.getSkillLevel(SkillFactory.getSkill(2301002))) / 100)
			...
			for (MapleCharacter affected : affectedp) {
				...
				affected.getMap().broadcastMessage(affected,
						MaplePacketCreator.showBuffeffect(affected.getId(), sourceid, 2, (byte) 3), false);
			}
		}
	}
	...
	private void applyMonsterBuff(MapleCharacter applyfrom) {
		Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(), applyfrom.isFacingLeft());
		List<MapleMapObject> affected = applyfrom.getMap().getMapObjectsInRect(bounds,
				Arrays.asList(MapleMapObjectType.MONSTER));
		...
		for (MapleMapObject mo : affected) {
			...
			if (makeChanceResult()) {
				monster.applyStatus(applyfrom, new MonsterStatusEffect(getMonsterStati(), skill_, false), isPoison(),
						getDuration());
			}
			i++;
			if (i >= mobCount) {
	...
	private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
		...
		if (isPirateMorph()) {
			localstatups = Collections
					.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MORPH, getMorph(applyto)));
		}
		if (primary) {
			...
			if (isDash()) {
				if (!applyto.getJob().isA(MapleJob.PIRATE)) {
					...
					applyto.getClient().getSession()
							.write(MaplePacketCreator.giveDash(localstatups, localX, localY, seconds));
				}
			} else if (isInfusion()) {
				...
				applyto.getClient().getSession().write(MaplePacketCreator
						.giveBuff((skill ? localsourceid : -localsourceid), localDuration, localstatups));
			}
		} else {
			// Aapply empty buff icon.
			applyto.getClient().getSession()
					.write(MaplePacketCreator.giveBuffTest((skill ? localsourceid : -localsourceid), localDuration, 0));
		}
		if (isMonsterRiding()) {
				...
				applyto.getMap().broadcastMessage(applyto,
						MaplePacketCreator.showMonsterRiding(applyto.getId(),
								Collections.singletonList(
										new Pair<MapleBuffStat, Integer>(MapleBuffStat.MONSTER_RIDING, 1)),
								givemount),
						false);
			}
		...
		if (isDs()) {
			...
			applyto.getMap().broadcastMessage(applyto,
					MaplePacketCreator.giveForeignBuff(applyto.getId(), dsstat, this), false);
		}
		if (isCombo()) {
			List<Pair<MapleBuffStat, Integer>> stat = Collections
					.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.COMBO, 1));
			applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this),
					false);
		}
		if (isShadowPartner()) {
			List<Pair<MapleBuffStat, Integer>> stat = Collections
					.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SHADOWPARTNER, 0));
			applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this),
					false);
		}
		if (isSoulArrow()) {
			List<Pair<MapleBuffStat, Integer>> stat = Collections
					.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.SOULARROW, 0));
			applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this),
					false);
		}
		if (isEnrage()) {
		...
		if (primary) {
			if (isDash()) {
				applyto.getMap().broadcastMessage(applyto,
						MaplePacketCreator.showDashEffecttoOthers(applyto.getId(), localX, localY, seconds), false);
			} else if (isInfusion()) {
				applyto.getMap().broadcastMessage(applyto,
						MaplePacketCreator.giveForeignInfusion(applyto.getId(), x, seconds), false);
			} else {
				applyto.getMap().broadcastMessage(applyto,
						MaplePacketCreator.showBuffeffect(applyto.getId(), sourceid, 1, (byte) 3), false);
			}
	...
	private boolean isPartyBuff() {
		...
		if ((sourceid >= 1211003 && sourceid <= 1211008) || sourceid == 1221003 || sourceid == 1221004) { // wk
																											// charges
																											// have
																											// lt
																											// and
	...
	private boolean isHeroWill() {
		return skill && (sourceid == 1121011 || sourceid == 1221012 || sourceid == 1321010 || sourceid == 2121008
				|| sourceid == 2221008 || sourceid == 2321009 || sourceid == 3121009 || sourceid == 3221008
				|| sourceid == 4121009 || sourceid == 4221008 || sourceid == 5121008 || sourceid == 5221010);
	}
