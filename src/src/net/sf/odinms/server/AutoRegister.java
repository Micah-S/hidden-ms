...
public class AutoRegister {
	...
	public static void createAccount(String login, String pwd, String eip) {
		try {
			PreparedStatement ipq = DatabaseConnection.getConnection()
					.prepareStatement("SELECT lastknownip FROM accounts WHERE lastknownip = ?");
			...
			if (!rs.first() || rs.last() && rs.getRow() < LoginServer.getInstance().AutoRegLimit()) {
				try {
					...
					System.out.println(
							"XSource: Error creating the account of (" + login + " | " + pwd + " | " + eip + ").");
				}
			}
			rs.close();
			ipq.close();
