...
public class MapleInventoryManipulator {
	...
	public static boolean addById(MapleClient c, int itemId, short quantity, String owner, int petid) {
		if (quantity >= 4000 || quantity < 0) {
			AutobanManager.getInstance().autoban(c.getPlayer().getClient(),
					"XSource| PE Item: " + quantity + "x " + itemId);
			return false;
		}
		MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
		MapleInventoryType type = ii.getInventoryType(itemId);
	...
	public static boolean addFromDrop(MapleClient c, IItem item, boolean show, String owner) {
		...
		if (!c.getChannelServer().allowMoreThanOne() && ii.isPickupRestricted(item.getItemId())
		...
		if (quantity >= 4000 || quantity < 0) {
			AutobanManager.getInstance().autoban(c.getPlayer().getClient(),
					"XSource| PE Item: " + quantity + "x " + item.getItemId());
			return false;
		}
		if (!type.equals(MapleInventoryType.EQUIP)) {
	...
	public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
		...
		if (quantity < 0 || source == null || quantity == 0 && !ii.isThrowingStar(itemId) && !ii.isBullet(itemId)) {
			System.out.println("Dropping " + quantity + " " + (source == null ? "?" : itemId) + " (" + type.name() + "/"
					+ src + ")");
			c.disconnect();
			return;
		}
		...
		if (quantity < source.getQuantity() && !ii.isThrowingStar(itemId) && !ii.isBullet(itemId)) {
			...
			boolean weddingRing = source.getItemId() == 1112803 || source.getItemId() == 1112806
					|| source.getItemId() == 1112807 || source.getItemId() == 1112809;
			if (weddingRing) {
