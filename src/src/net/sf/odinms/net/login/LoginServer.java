			...
			synchronized (lwi) {
				...
				synchronized (wli) {
					try {
						...
						wli = worldRegistry.registerLoginServer(initialProp.getProperty("net.sf.odinms.login.key"),
								lwi);
						...
						serverCheck = Boolean
								.parseBoolean(prop.getProperty("net.sf.odinms.login.serverCheck", "false"));
	...
	public void run() {
		...
		try {
			acceptor.bind(new InetSocketAddress(PORT),
					new MapleServerHandler(PacketProcessor.getProcessor(PacketProcessor.Mode.LOGINSERVER)), cfg);
