...
public class ServerlistRequestHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		c.getSession().write(MaplePacketCreator.getServerList(0, LoginServer.getInstance().getServerName(),
				LoginServer.getInstance().getLoad()));
		if (LoginServer.getInstance().twoWorldsActive())
			c.getSession().write(MaplePacketCreator.getServerList(1, LoginServer.getInstance().getServerName(),
					LoginServer.getInstance().getLoad()));
