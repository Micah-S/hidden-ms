			...
			switch (itemType) {
				case 505: // AP/SP reset
					if (itemId > 5050000) {
						...
						if (curLevel + 1 <= maxlevel && curLevelSPFrom > 0) {
							player.changeSkillLevel(skillSPFrom, curLevelSPFrom - 1,
						...
						switch (APFrom) {
							case 64: // str
								if (player.getStr() <= 4
										|| ((player.getJob().getId() / 100) == 1 && player.getStr() <= 35)) {
									break;
								}
								...
								break;
							case 128: // dex
								if (player.getDex() <= 4 || (((player.getJob().getId() / 100) == 4
										|| (player.getJob().getId() / 100) == 3) && player.getDex() <= 25)) {
									break;
								}
							...
							case 256: // int
								if (player.getInt() <= 4
										|| ((player.getJob().getId() / 100) == 2 && player.getInt() <= 20)) {
									break;
								}
							...
							case 512: // luk
								if (player.getLuk() <= 4
										|| ((player.getJob().getId() / 100) == 4 && player.getLuk() <= 35)) {
									break;
								}
						...
						switch (itemId / 1000 % 10) {
							case 1: // Megaphone
								if (player.getLevel() >= 10) {
									player.getMap().broadcastMessage(MaplePacketCreator.serverNotice(2,
											player.getName() + " : " + slea.readMapleAsciiString()));
									MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot,
											(short) 1, false);
								} else {
							...
							case 2: // Super megaphone
								...
								MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1,
										false);
								break;
							...
							case 5: // Maple TV
								...
								if (tvType != 1) {
									...
									if (tvType != 4) {
										...
										if (name.length() > 0) {
											...
											victim = net.sf.odinms.net.channel.ChannelServer.getInstance(channel)
								...
								if (!MapleTVEffect.active) {
									if (megassenger) {
										c.getChannelServer().getWorldInterface().broadcastMessage(null,
												MaplePacketCreator
														.serverNotice(3, c.getChannel(),
																player.getName() + " : " + builder.toString(), ear)
														.getBytes());
									}
				...
				case 539:
					if (player.getCanSmega() && player.getSmegaEnabled() && !player.getCheatTracker().Spam(3000, 3)) {
						...
						c.getChannelServer().getWorldInterface().broadcastSMega(null, MaplePacketCreator
								.getAvatarMega(c.getPlayer(), c.getChannel(), itemId, lines, (slea.readByte() != 0))
								.getBytes());
						MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1, false);
				...
				case 520:
					if (ii.getMeso(itemId) + player.getMeso() < Integer.MAX_VALUE) {
						...
						MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1, false);
						c.getSession().write(MaplePacketCreator.enableActions()); // do we really
																					// need this?
					} else {
				...
				case 510:
					player.getMap().broadcastMessage(c.getPlayer(),
							MaplePacketCreator.musicChange("Jukebox/Congratulation"), true);
					break;

				case 512:
					player.getMap().startMapEffect(ii.getMsg(itemId).replaceFirst("%s", player.getName())
							.replaceFirst("%s", slea.readMapleAsciiString()), itemId);
					MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1, false);
					break;
					...
					for (int i = 0; i < 3; i++) {
						...
						if (player.getInventory(MapleInventoryType.CASH).getItem(slot) != null) {
							...
							if (itemId == 5240012 && petID >= 5000028 && petID <= 5000033
									...
									|| itemId == 5240006 && (petID == 5000003 || petID == 5000007
											|| petID >= 5000009 && petID <= 5000010 || petID == 5000012
											|| petID == 5000044)) {
								pet.setFullness(100);
								...
								while (pet.getCloseness() >= ExpTable.getClosenessNeededForLevel(pet.getLevel() + 1)) {
									...
									player.getMap().broadcastMessage(
											MaplePacketCreator.showPetLevelUp(c.getPlayer(), player.getPetIndex(pet)));
								}
								c.getSession().write(MaplePacketCreator.updatePet(pet, true));
								player.getMap().broadcastMessage(c.getPlayer(),
										MaplePacketCreator.commandResponse(player.getId(), (byte) 1, 0, true, true),
										true);
								MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1,
										false);
				...
				case 528:
					if (itemId == 5281000) { // Credits to purplemadness
						Rectangle bounds = new Rectangle((int) player.getPosition().getX(),
								(int) player.getPosition().getY(), 1, 1);
						...
						player.getMap().broadcastMessage(
								MaplePacketCreator.getChatText(player.getId(), "Oh no, I farted!", false, 1));
					}
					break;
				...
				case 517:
					...
					if (pet != null) {
						...
						if (newName.length() > 2 && newName.length() < 14) {
							...
							c.getSession().write(MaplePacketCreator.enableActions());
							player.getMap().broadcastMessage(c.getPlayer(),
									MaplePacketCreator.changePetName(c.getPlayer(), newName, 1), true);
							MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, slot, (short) 1,
									false);
						} else {
				...
				case 504:
					...
					if (rocktype == 0x00) {
						...
						if (victim == null) { // player dcs / ccs while you cc o____o
							...
							if ((victim.isGM() && player.isGM()) || !victim.isGM()) { // I should
																						// really
																						// handle
																						// this
																						// before
																						// the
																						// switch
								if ((player.getMap().getMapName().equals(victim.getMap().getMapName()) && !vip)
										|| vip) {
									player.changeMap(map, map.findClosestSpawnpoint(victim.getPosition()));
								} else {
									player.dropMessage(1,
											"You cannot warp to this player because he's not in the same continent.");
								}
							} else {
								player.dropMessage(1, "You cannot use VIP teleport rocks on a GM.");
							}
