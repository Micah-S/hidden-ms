	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (toUse != null && toUse.getQuantity() == 1) {
			...
			if (skilldata.get("skillid") == 0) { // Wrong Job.
				canuse = false;
			} else if (player.getMasterLevel(SkillFactory.getSkill(skilldata.get("skillid"))) >= skilldata
					.get("reqSkillLevel") || skilldata.get("reqSkillLevel") == 0) {
				canuse = true;
			...
			player.getClient().getSession()
					.write(MaplePacketCreator.skillBookSuccess(player, skill, maxlevel, canuse, success));
		} else {
			return;
		}
	}
