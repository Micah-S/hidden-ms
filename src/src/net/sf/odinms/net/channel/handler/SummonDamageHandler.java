...
public class SummonDamageHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		player.getMap().broadcastMessage(player,
				MaplePacketCreator.summonAttack(player.getId(), summon.getSkill(), summon.getStance(), allDamage),
				summon.getPosition());
		for (SummonAttackEntry attackEntry : allDamage) {
			...
			if (target != null) {
				if (damage >= 100000000) {
					AutobanManager.getInstance().autoban(player.getClient(), "XSource| " + player.getName()
							+ "'s summon dealt " + damage + " to monster " + target.getId() + ".");
				}
				if (damage > 0 && summonEffect.getMonsterStati().size() > 0) {
					if (summonEffect.makeChanceResult()) {
						MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
								summonEffect.getMonsterStati(), summonSkill, false);
