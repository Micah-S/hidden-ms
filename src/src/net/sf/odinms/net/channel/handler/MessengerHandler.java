						...
						try {
							...
							if (messenger != null) {
								if (messenger.getMembers().size() < 3) {
									...
									wci.joinMessenger(messenger.getId(), messengerplayer, player.getName(),
				...
				if (messenger.getMembers().size() < 3) {
					...
					if (target != null) {
						if (target.getMessenger() == null) {
							...
							c.getSession().write(MaplePacketCreator.messengerChat(
									player.getName() + " : " + input + " is already using Maple Messenger"));
						}
					} else {
						try {
							if (ChannelServer.getInstance(c.getChannel()).getWorldInterface().isConnected(input)) {
								ChannelServer.getInstance(c.getChannel()).getWorldInterface().messengerInvite(
										c.getPlayer().getName(), messenger.getId(), input, c.getChannel());
							} else {
					...
					c.getSession().write(MaplePacketCreator.messengerChat(
							player.getName() + " : You cannot have more than 3 people in the Maple Messenger"));
				}
				break;
			case 0x05: // decline
