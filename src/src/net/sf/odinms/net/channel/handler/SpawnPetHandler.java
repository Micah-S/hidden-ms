...
public class SpawnPetHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (item.getItemId() == 5000028 || item.getItemId() == 5000047) {
			...
			for (int i = 0; i < pet.length && !done; i++) {
				...
				if (!player.haveItem(pet[petno], 1, true, true)) {
					MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.CASH, item.getPosition(), (short) 1,
							true, false);
		...
		MaplePet pet = MaplePet.loadFromDb(player.getInventory(MapleInventoryType.CASH).getItem(slot).getItemId(), slot,
				player.getInventory(MapleInventoryType.CASH).getItem(slot).getPetId());
		if (pet == null) {
			MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, item.getItemId(),
					(short) item.getQuantity(), false, false);
