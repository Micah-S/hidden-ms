...
public class MTSHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (slea.available() > 0) {
			...
			if (op == 2) { // Place item for sale.
				...
				if (i.getQuantity() > 0 && i != null && c.getPlayer().getMeso() >= 1337) {
					...
					try {
						...
						ps = con.prepareStatement(
								"SELECT count(*) FROM mts_items WHERE seller = " + c.getPlayer().getId());
				...
				c.getSession().write(MaplePacketCreator.enableMTS());
				c.getSession().write(getMTS(c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(),
				...
				try {
					...
					if (rs.next()) {
						...
						if (rs.getInt("type") != 1) {
							...
							ii.setPosition(c.getPlayer().getInventory(
									MapleItemInformationProvider.getInstance().getInventoryType(rs.getInt("itemid")))
									.getNextFreeSlot());
							i = ii.copy();
						} else {
							...
							equip.setPosition(c.getPlayer().getInventory(
									MapleItemInformationProvider.getInstance().getInventoryType(rs.getInt("itemid")))
									.getNextFreeSlot());
							i = equip.copy();
						}
				...
				try {
					...
					if (rs1.next()) {
						...
						if (!rs.next()) {
							...
							pse.close();
							ps.close();
							// c.getSession().write(MaplePacketCreator.addToCartMessage(false,
							// false));
						} else {
							rs.close();
							ps.close();
				...
				/*
				 * c.getSession().write(getMTS(c.getPlayer().getCurrentTab(),
				 * c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
				 * c.getSession().write(MaplePacketCreator.enableMTS());
				 * c.getSession().write(MaplePacketCreator.enableActions());
				 * c.getSession().write(MaplePacketCreator.TransferInventory(getTransfer(c.getPlayer
				 * ().getId())));
				 * c.getSession().write(MaplePacketCreator.NotYetSoldInv(getNotYetSold(c.getPlayer()
				 * .getId())));
				 */
				...
				try {
					...
					if (rs.next()) {
						...
						if (c.getPlayer().getCSPoints(1) >= price) {
							...
							if (alwaysnull) {
								...
								if (rse.next()) {
									PreparedStatement psee = con.prepareStatement(
											"UPDATE accounts SET paypalNX = paypalNX + ? WHERE id = ?");
							...
							c.getSession().write(MaplePacketCreator.showMTSCash(c.getPlayer()));
							c.getSession()
									.write(MaplePacketCreator.TransferInventory(getTransfer(c.getPlayer().getId())));
							c.getSession()
									.write(MaplePacketCreator.NotYetSoldInv(getNotYetSold(c.getPlayer().getId())));
							c.getSession().write(MaplePacketCreator.enableActions());
						} else {
		...
		try {
			...
			while (rs.next()) {
				if (rs.getInt("type") != 1) {
					...
					items.add(new MTSItemInfo((IItem) i, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"),
					...
					equip.setLocked((byte) rs.getInt("locked"));
					equip.setLevel((byte) rs.getInt("level"));
					items.add(new MTSItemInfo((IItem) equip, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"),
							rs.getString("sellername"), rs.getString("sell_ends")));
				}
			}
			rs.close();
			...
			while (rs.next()) {
				...
				if (rse.next()) {
					if (rse.getInt("type") != 1) {
						...
						items.add(new MTSItemInfo((IItem) i, rse.getInt("price"), rse.getInt("id"),
						...
						items.add(new MTSItemInfo((IItem) equip, rse.getInt("price"), rse.getInt("id"),
								rse.getInt("seller"), rse.getString("sellername"), rse.getString("sell_ends")));
					}
				}
			}
	...
	public MaplePacket getMTSSearch(int tab, int type, int cOi, String search, int page) {
		...
		try {
			...
			ps.close();
			if (type != 0) {
				ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? " + listaitems
