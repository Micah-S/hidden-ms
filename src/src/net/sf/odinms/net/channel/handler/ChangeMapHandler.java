...
public class ChangeMapHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (slea.available() == 0) {
			...
			try {
				MaplePacket packet = MaplePacketCreator.getChannelChange(InetAddress.getByName(socket[0]),
			...
			if (player.getBuffedValue(MapleBuffStat.MORPH) != null
					&& player.getBuffedValue(MapleBuffStat.COMBO) != null) {
			...
			if (targetid != -1 && !c.getPlayer().isAlive()) {
				...
				if (portal != null) {
					...
					/*
					 * if (player.getClan() == -1 && !c.isGuest()) { for (int i = 0; i < 3; i++) {
					 * player.dropMessage(6,
					 * "You have yet to join a clan ! Type @clan to join a clan.."); } }
					 */
				} else {
