...
public abstract class AbstractMovementPacketHandler extends AbstractMaplePacketHandler {
	...
	protected List<LifeMovementFragment> parseMovement(LittleEndianAccessor lea) {
		...
		for (int i = 0; i < numCommands; i++) {
			...
			switch (command) {
				...
				case 17: // Float
				{
					...
					// log.trace("Move to {},{} command {} wobble {},{} ? {} state {} duration {}",
					// new Object[] { xpos,
				...
				case 16: // Float
				{
					...
					int newstate = lea.readByte();
					int duration = lea.readShort();
					RelativeLifeMovement rlm = new RelativeLifeMovement(command, new Point(xmod, ymod), duration,
							newstate);
					res.add(rlm);
					// log.trace("Relative move {},{} state {}, duration {}", new Object[] { xmod,
					// ymod, newstate,
					// duration });
					break;
				}
		...
		if (numCommands != res.size()) {
			// log.warn("numCommands ({}) does not match the number of deserialized movement
			// commands ({})", numCommands, res.size());
		}
		return res;
	}
