...
public class MoveLifeHandler extends AbstractMovementPacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mmo == null || mmo.getType() != MapleMapObjectType.MONSTER) {
			/*
			 * if (mmo != null) { log.
			 * warn("[dc] Player {} is trying to move something which is not a monster. It is a {}."
			 * , new Object[] { c.getPlayer().getName(),
			 * c.getPlayer().getMap().getMapObject(objectid).getClass().getCanonicalName() }); }
			 */
			return;
		}
		...
		if (toUse != null) {
			c.getSession().write(MaplePacketCreator.moveMonsterResponse(objectid, moveid, monster.getMp(), aggro,
					toUse.getSkillId(), toUse.getSkillLevel()));
		} else {
			c.getSession().write(MaplePacketCreator.moveMonsterResponse(objectid, moveid, monster.getMp(), aggro));
		}
		if (aggro) {
		...
		if (res != null) {
			...
			MaplePacket packet = MaplePacketCreator.moveMonster(skillByte, skill, skill_1, skill_2, skill_3, objectid,
					startPos, res);
