	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
			...
			for (MapleMapObject mmo : player.getMap().getMapObjects()) {
				if (mmo instanceof MapleMist) {
					...
					if (mist.getSourceSkill().getId() == 4221006) { // Smokescreen
						for (MapleMapObject mmoplayer : player.getMap().getMapObjectsInRect(mist.getBox(),
		...
		if (damage < -1 || damage > 100000) {
			AutobanManager.getInstance().autoban(player.getClient(),
					"XSource| " + player.getName() + " took " + damage + " of damage.");
			...
			if (!player.isHidden() && player.isAlive()) {
				...
				if (attacker != null && !attacker.isBoss()) {
					if (damagefrom == -1 && player.getBuffedValue(MapleBuffStat.POWERGUARD) != null) {
						...
						damage -= bouncedamage;
						player.getMap().broadcastMessage(player, MaplePacketCreator.damageMonster(oid, bouncedamage),
								false, true);
					...
					if ((damagefrom == 0 || damagefrom == 1)
						...
						for (int manaReflect : manaReflectSkillId) {
							...
							if (player.isBuffFrom(MapleBuffStat.MANA_REFLECTION, manaReflectSkill)
									&& player.getSkillLevel(manaReflectSkill) > 0 && manaReflectSkill
											.getEffect(player.getSkillLevel(manaReflectSkill)).makeChanceResult()) {
								int bouncedamage = (int) (damage
										* (manaReflectSkill.getEffect(player.getSkillLevel(manaReflectSkill)).getX()
												/ 100));
								...
								player.getMap().broadcastMessage(player,
										MaplePacketCreator.damageMonster(oid, bouncedamage), true);
								player.getClient().getSession()
										.write(MaplePacketCreator.showOwnBuffEffect(manaReflect, 5));
								player.getMap().broadcastMessage(player,
										MaplePacketCreator.showBuffeffect(player.getId(), manaReflect, 5, (byte) 3),
										false);
								break;
							}
						}
					...
					try {
						...
						for (int achilles : achillesSkillId) {
							...
							if (player.getSkillLevel(achillesSkill) > 0) {
								double multiplier = achillesSkill.getEffect(player.getSkillLevel(achillesSkill)).getX()
										/ 1000.0;
				...
				if (c.getPlayer().getMap().getId() == 980010101) {
					if (monsteridfrom == 9300166) {
						...
						if (player.getBombPoints() < 1) {
							...
							c.getPlayer().dropMessage("[" + c.getChannelServer().getServerName()
									+ "] You have died in Battle at the Bomberman Arena.");
							c.getPlayer().getMap().broadcastMessage(
									MaplePacketCreator.serverNotice(1, "[" + c.getChannelServer().getServerName()
											+ "] The person " + player.getName() + " has died in Bomberman PvP."));
							return;
						} else {
							c.getPlayer().getMap()
									.broadcastMessage(MaplePacketCreator.serverNotice(1,
											"[" + c.getChannelServer().getServerName() + "] The player "
													+ player.getName() + " now has " + player.getBombPoints()
					...
					player.getMap()
							.broadcastMessage(player, MaplePacketCreator.damagePlayer(damagefrom, monsteridfrom,
									player.getId(), damage, fake, direction, is_pgmr, pgmr, is_pg, oid, pos_x, pos_y),
									false);
				}
			}
		}
