		...
		if (effect.getCooldown() > 0) {
			if (c.getPlayer().skillisCooling(skillid)) {
				...
				ScheduledFuture<?> timer = TimerManager.getInstance()
						.schedule(new CancelCooldownAction(c.getPlayer(), skillid), effect.getCooldown() * 1000);
		...
		try {
			switch (skillid) {
				...
				case 1321001:
					...
					c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
							MaplePacketCreator.showBuffeffect(c.getPlayer().getId(), skillid, 1, direction), false);
					for (FakeCharacter ch : c.getPlayer().getFakeChars()) {
						ch.getFakeChar().getMap().broadcastMessage(ch.getFakeChar(),
								MaplePacketCreator.showBuffeffect(ch.getFakeChar().getId(), skillid, 1, direction),
								false);
					}
					c.getSession().write(MaplePacketCreator.enableActions());
					break;
			...
			if (c.getPlayer().isAlive()) {
				if (skill.getId() != 2311002 || c.getPlayer().canDoor()) {
					...
					new ServernoticeMapleClientMessageCallback(5, c)
							.dropMessage("Please wait 5 seconds before casting Mystic Door again.");
					c.getSession().write(MaplePacketCreator.enableActions());
				}
			} else {
				c.getSession().write(MaplePacketCreator.enableActions());
