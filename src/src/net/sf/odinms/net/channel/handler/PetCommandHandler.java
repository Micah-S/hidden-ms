...
public class PetCommandHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
			...
			if (pet.getCloseness() < 30000) {
				...
				if (newCloseness >= ExpTable.getClosenessNeededForLevel(pet.getLevel() + 1)) {
					...
					c.getPlayer().getMap().broadcastMessage(
							MaplePacketCreator.showPetLevelUp(c.getPlayer(), c.getPlayer().getPetIndex(pet)));
				}
		...
		player.getMap().broadcastMessage(player,
				MaplePacketCreator.commandResponse(player.getId(), command, petIndex, success, false), true);
	}
}