		...
		switch (operation) {
			case 1: { // Create.
				if (c.getPlayer().getParty() == null) {
					...
					c.getSession().write(
							MaplePacketCreator.serverNotice(5, "You can't create a party as you are already in one."));
				}
				break;
			}
			case 2: { // Leave.
			...
			case 3: { // Accept invitation.
				...
				if (c.getPlayer().getParty() == null) {
					try {
						...
						if (party != null) {
							...
							c.getSession().write(MaplePacketCreator.serverNotice(5,
									"The party you are trying to join does not exist."));
						}
					} catch (Exception e) {
						c.getChannelServer().reconnectWorld();
					}
					...
					c.getSession().write(
							MaplePacketCreator.serverNotice(5, "You can't join the party as you are already in one."));
				}
				break;
			}
			case 4: { // Invite.
