	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (emote > 7) {
			...
			if (iv.findById(emoteid) == null) {
				// log.info("[h4x] Player {} is using a face expression he does not have: {}",
				// c.getPlayer().getName(), Integer.valueOf(emoteid));
		...
		for (FakeCharacter ch : c.getPlayer().getFakeChars()) {
			c.getPlayer().getMap().broadcastMessage(ch.getFakeChar(),
					MaplePacketCreator.facialExpression(ch.getFakeChar(), emote), false);
		}
		c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
				MaplePacketCreator.facialExpression(c.getPlayer(), emote), false);
	}
}