		...
		try {
			...
			switch (slea.readByte()) {
				case 0x0A:
					...
					c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(),
							MaplePacketCreator.allianceNotice(alliance.getId(), notice), -1, -1);
					break;
				case 0x08:
					...
					c.getChannelServer().getWorldInterface().allianceMessage(alliance.getId(),
							MaplePacketCreator.changeAllianceRankTitle(alliance.getId(), ranks), -1, -1);
					break;
				case 0x03: // fall through to default.
					/*
					 * String charName = slea.readMapleAsciiString(); int channel =
					 * c.getChannelServer().getWorldInterface().find(charName); if (channel == -1) {
					 * c.getPlayer().dropMessage("The c.getPlayer() is not online"); } else {
					 * MapleCharacter victim =
					 * ChannelServer.getInstance(channel).getPlayerStorage().getCharacterByName(
					 * charName); if (victim.getGuildId() == 0) {
					 * c.getPlayer().dropMessage("c.getPlayer() does not have a guild"); } else if
					 * (victim.getGuildRank() != 1) {
