	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (channel == -1) {
			...
			if (mode == 6) { // Whisper
				...
				if (!(c.getPlayer().isGM()) && text.length() > 100) { // 70 = max text for client.
																		// remove this if u have
																		// edit the client
					AutobanManager.getInstance().autoban(c.getPlayer().getClient(),
							"XSource| " + c.getPlayer().getName() + " had infinite text with a text length of "
									+ text.length() + ".");
				} else {
				...
				if (!victim.isGM() || (c.getPlayer().isGM() && victim.isGM())) {
					if (victim.inCS()) {
						...
						c.getSession()
								.write(MaplePacketCreator.getFindReplyWithMap(victim.getName(), victim.getMapId()));
					} else {
						c.getSession().write(MaplePacketCreator.getFindReply(victim.getName(),
