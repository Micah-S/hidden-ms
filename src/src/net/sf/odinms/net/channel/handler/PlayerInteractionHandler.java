...
public class PlayerInteractionHandler extends AbstractMaplePacketHandler {
	...
	private enum Action {
		...
		CREATE(0), INVITE(2), DECLINE(3), VISIT(4), CHAT(6), EXIT(0xA), OPEN(0xB), SET_ITEMS(0xE), SET_MESO(
				0xF), CONFIRM(0x10), ADD_ITEM(0x14), BUY(0x15), REMOVE_ITEM(0x19), BAN_PLAYER(0x1A), PUT_ITEM(
						0x1F), MERCHANT_BUY(0x20), TAKE_ITEM_BACK(0x24), MAINTENANCE_OFF(0x25), MERCHANT_ORGANIZE(
								0x26), CLOSE_MERCHANT(0x27), REQUEST_TIE(44), ANSWER_TIE(45), GIVE_UP(
										46), EXIT_AFTER_GAME(50), CANCEL_EXIT(51), READY(
												52), UN_READY(53), MOVE_OMOK(58), START(55), SKIP(57), SELECT_CARD(62);
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == Action.CREATE.getCode()) {
			...
			if (createType == 3) { // Trade
				...
				if (createType == 1 || createType == 2) {
					...
					if (c.getPlayer().getMap()
							.getMapObjectsInRange(c.getPlayer().getPosition(), 19500,
									Arrays.asList(MapleMapObjectType.SHOP, MapleMapObjectType.HIRED_MERCHANT))
							.size() != 0 || c.getPlayer().getMapId() < 910000001
			...
			int type = slea.readByte(); // Piece ( 1 or 2; Owner has one piece, visitor has another,
										// it switches every game. )
			...
			if (turn == 1) {
				...
				game.broadcast(MaplePacketCreator.getMatchCardSelect(turn, slot, firstslot, turn),
				...
				game.broadcast(MaplePacketCreator.getMatchCardSelect(turn, slot, firstslot,
						game.isOwner(c.getPlayer()) ? 0 : 1), true);
			}
			...
			if (shop != null && shop.isOwner(c.getPlayer())) {
				if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
					...
					if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000 || checksmquantity < 0
							|| checkiquantity < 0 || checkiquantity > 20000) {
						AutobanManager.getInstance().autoban(c,
								"XSource| PE Player Shop item: " + sellItem.getItemId());
						return;
					}
