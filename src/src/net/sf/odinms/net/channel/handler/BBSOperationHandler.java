...
public class BBSOperationHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (!MapleGuild.ENABLE_BBS) {
			c.getSession().write(
					MaplePacketCreator.serverNotice(1, "Your server administrator has currently disabled Guild BBS."));
			return;
		}
	...
	public static void newBBSThread(MapleClient client, String title, String text, int icon, boolean bNotice) {
		...
		try {
			...
			ps = con.prepareStatement(
					"INSERT INTO bbs_threads " + "(`postercid`, `name`, `timestamp`, `icon`, `startpost`, "
	...
	public static void deleteBBSThread(MapleClient client, int localthreadid) {
		...
		try {
			...
			if (mc.getId() != threadRS.getInt("postercid") && mc.getGuildRank() > 2) { // but let
																						// all
																						// master/jrs
																						// delete
				threadRS.close();
	...
	public static void displayThread(MapleClient client, int threadid, boolean bIsThreadIdLocal) {
		...
		try {
			...
			log.error(
					"The number of reply rows does not match the replycount in thread. Thread Id = " + re.getMessage(),
					re);
			try {
