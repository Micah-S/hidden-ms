...
public class DueyHandler extends AbstractMaplePacketHandler {
	...
	public enum Actions {
		...
		TOSERVER_SEND_ITEM(0x02), TOSERVER_CLOSE_DUEY(0x07), TOSERVER_CLAIM_PACKAGE(0x04), TOSERVER_REMOVE_PACKAGE(
				0x05), TOCLIENT_OPEN_DUEY(0x08), TOCLIENT_NOT_ENOUGH_MESOS(0x0A), TOCLIENT_NAME_DOES_NOT_EXIST(
						0x0C), TOCLIENT_SAMEACC_ERROR(0x0D), TOCLIENT_PACKAGE_MSG(0x1B), TOCLIENT_SUCCESSFUL_MSG(0x17), // Ending
																														// byte;
																														// 4
																														// if
																														// recieved.
			...
			if (c.getPlayer().getMeso() >= finalcost) {
				...
				if (accid != -1) {
					if (accid != c.getAccID()) {
						...
						c.getSession()
								.write(MaplePacketCreator.sendDueyMSG(Actions.TOCLIENT_SUCCESSFULLY_SENT.getCode()));
						send = true;
					} else {
					...
					c.getSession()
							.write(MaplePacketCreator.sendDueyMSG(Actions.TOCLIENT_NAME_DOES_NOT_EXIST.getCode()));
				}
			} else {
			...
			if (send) {
				if (inventId > 0) {
					...
					if (item != null && c.getPlayer().haveItem(item.getItemId(), amount, false, true)) {
						...
						addItemToDB(item, (short) amount, mesos, c.getPlayer().getName(),
								MapleCharacter.getIdByName(recipient, 0), recipientOn);
					} else { // hax.
						c.disconnect();
						c.getSession().close();
						return;
	...
	public static List<DueyPackages> loadItems(MapleCharacter chr) {
		...
		try {
			PreparedStatement ps = con.prepareStatement(
					"SELECT * FROM dueypackages LEFT JOIN dueyitems USING (PackageId) WHERE RecieverId = ?"); // PLEASE
																												// WORK
																												// D:
	...
	public static DueyPackages loadSingleItem(int packageid) {
		...
		try {
			PreparedStatement ps = con.prepareStatement(
					"SELECT * FROM dueypackages LEFT JOIN dueyitems USING (PackageId) WHERE PackageId = ?"); // PLEASE
																												// WORK
																												// D:
	...
	public static int dueyStorageSize(MapleCharacter chr) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT COUNT(*) as dueypackages FROM Duey WHERE RecieverId = ?");
			ps.setInt(1, chr.getId());
			ResultSet rs = ps.executeQuery();
			rs.next();
