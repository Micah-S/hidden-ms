...
public class ChannelServer implements Runnable, ChannelServerMBean {
	...
	private ChannelServer(String key) {
		mapFactory = new MapleMapFactory(
				MapleDataProviderFactory
						.getDataProvider(new File(System.getProperty("net.sf.odinms.wzpath") + "/Map.wz")),
				MapleDataProviderFactory
			...
			synchronized (cwi) {
				...
				synchronized (wci) {
					...
					try {
						...
						itemStatMultiplier = Short
								.parseShort(props.getProperty("net.sf.odinms.world.itemStatMultiplier"));
						...
						extraCommands = Boolean
								.parseBoolean(props.getProperty("net.sf.odinms.world.extraCommands", "false"));
	...
	public void run() {
		...
		// cfg.setThreadModel(ThreadModel.MANUAL); // *fingers crossed*, I hope the executor filter
		// handles everything
	...
	public void shutdown(int time) {
		broadcastPacket(MaplePacketCreator.serverNotice(0,
				"The world will be shut down in " + (time / 60000) + " minutes, please log off safely"));
			...
			public void run() {
				for (ChannelServer channel : getAllInstances()) {
					for (int i = 910000001; i <= 910000022; i++) {
						for (MapleMapObject obj : channel.getMapFactory().getMap(i).getMapObjectsInRange(
