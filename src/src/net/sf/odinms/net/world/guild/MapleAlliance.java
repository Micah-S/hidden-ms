...
public class MapleAlliance implements java.io.Serializable {
	...
	public static void disbandAlliance(MapleClient c, int allianceId) {
		...
		try {
			c.getChannelServer().getWorldInterface().allianceMessage(c.getPlayer().getGuild().getAllianceId(),
	...
	public static MapleAlliance createAlliance(MapleCharacter chr1, MapleCharacter chr2, String name) {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO `alliance` (`name`, `guild1`, `guild2`) VALUES (?, ?, ?)");
			ps.setString(1, name);
			ps.setInt(2, guild1);
			ps.setInt(3, guild2);
			ps.executeUpdate();
