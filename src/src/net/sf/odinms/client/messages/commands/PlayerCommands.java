...
public class PlayerCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
		...
		if (splitted[0].equals("@command") || splitted[0].equals("@commands") || splitted[0].equals("@help")) {
			...
			if (player.getMeso() >= 10000000) {
				...
				String text = (!player.getSmegaEnabled() ? "[Disable] Smegas are now disable."
						: "[Enable] Smegas are now enable.");
				mc.dropMessage(text);
				...
				try {
					c.getChannelServer().getWorldInterface()
							.broadcastGMMessage(null,
									MaplePacketCreator.serverNotice(6, "Channel: " + c.getChannel() + "  "
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] { new CommandDefinition("command", 0), new CommandDefinition("commands", 0),
				new CommandDefinition("help", 0), new CommandDefinition("checkstats", 0),
				new CommandDefinition("save", 0), new CommandDefinition("expfix", 0),
				new CommandDefinition("dispose", 0), new CommandDefinition("emo", 0),
				new CommandDefinition("rebirth", 0), new CommandDefinition("reborn", 0),
				new CommandDefinition("togglesmega", 0), new CommandDefinition("str", 0),
				new CommandDefinition("dex", 0), new CommandDefinition("int", 0), new CommandDefinition("luk", 0),
				new CommandDefinition("hp", 0), new CommandDefinition("mp", 0), new CommandDefinition("gm", 0),
				new CommandDefinition("revive", 0), new CommandDefinition("afk", 0),
