...
public class Donator implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
		...
		if (splitted[0].equals("!buffme")) {
			int[] array = { 9001000, 9101002, 9101003, 9101008, 2001002, 1101007, 1005, 2301003, 5121009, 1111002,
					4111001, 4111002, 4211003, 4211005, 1321000, 2321004, 3121002 };
			for (int i = 0; i < array.length; i++) {
				SkillFactory.getSkill(array[i]).getEffect(SkillFactory.getSkill(array[i]).getMaxLevel())
						.applyTo(player);
			}
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] { new CommandDefinition("buffme", 1), new CommandDefinition("goto", 1),
				new CommandDefinition("sexchange", 1), new CommandDefinition("storage", 1) };
	}
}