...
public class MapleRing implements Comparable<MapleRing> {
	...
	public static MapleRing loadFromDb(int ringId) {
		try {
			...
			rs.next();
			MapleRing ret = new MapleRing(ringId, rs.getInt("partnerRingId"), rs.getInt("partnerChrId"),
	...
	public static int createRing(int itemid, final MapleCharacter partner1, final MapleCharacter partner2) {
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("INSERT INTO rings (itemid, partnerChrId, partnername) VALUES (?, ?, ?)");
			ps.setInt(1, itemid);
