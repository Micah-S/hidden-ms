...
public class MapleClient {
	...
	private Map<Pair<MapleCharacter, Integer>, Integer>	timesTalked				= new HashMap<Pair<MapleCharacter, Integer>, Integer>();	// npcid,
																																			// times
		...
		if (success) {
			...
			try {
				...
				PreparedStatement ps = con
						.prepareStatement("UPDATE accounts SET LastLoginInMilliseconds = ? WHERE id = ?");
		...
		if (chr != null && isLoggedIn()) {
			...
			try {
				...
				if (chr.getGuildId() > 0) {
					...
					if (allianceId > 0) {
						wci.allianceMessage(allianceId, MaplePacketCreator.allianceMemberOnline(chr, false),
								chr.getId(), -1);
					}
				}
			} catch (RemoteException e) {
	...
	public boolean deleteCharacter(int cid) {
		...
		try {
			...
			if (rs.getInt("guildid") > 0) {
				MapleGuildCharacter mgc = new MapleGuildCharacter(cid, 0, rs.getString("name"), -1, 0,
				...
				try {
					if (lastPong - then < 0) {
						if (getSession().isConnected()) {
							System.out
									.println("Auto DC : " + session.getRemoteAddress().toString() + " : Ping Timeout.");
							getSession().close();
						}
					}
