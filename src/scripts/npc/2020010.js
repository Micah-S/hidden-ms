...
/*
	...
	Description: 		Bowman 3rd Job Advancement
    ...
    if (actionx["Mental"]) {
        if (status == 0)
            ...
            if (cm.getJob().equals(MapleJob.HUNTER)) {
                ...
                cm.sendNext("Great! You are now a #bRanger#k. As a Ranger, you will learn some of the most sophisticated skills related to ranged-based attacks.");
            }
            if (cm.getJob().equals(MapleJob.CROSSBOWMAN)) {
                cm.changeJob(MapleJob.SNIPER);
                cm.getChar().gainAp(5);
                cm.gainItem(4031058, -1);
				cm.sendNext("Great! You are now a #bSniper#k. As a Sniper, you will learn some of the most sophisticated skills related to ranged-based attacks.")
            }
			cm.completeQuest(100102);
        } else if (status == 3) {
			cm.sendOk("I have also given you Skill Points and Ability Points, so please apply them when you get a chance. Now that you have made the job advancement, I believe you have now become a formidable Archer. Remember, though, that this will open up a whole new set of difficult journeys for you to take.")
        }
		...
		if (sel == undefined)
			sel = selection;
		if (sel == 0){
			if (cm.getPlayer().getLevel() >= 70 && cm.getJob().equals(MapleJob.HUNTER) || cm.getJob().equals(MapleJob.CROSSBOWMAN)) {
				if (status == 0) {
					cm.sendNext("Hmm. So you want to become a stronger Archer by making the 3rd job advancement? First let me say congratulations! Few have this level of dedication...");
				} else if (status == 1) {
					...
					cm.sendNext("Great! Now you will have to prove your strength and intelligence. Let me first explain the strength test. Do you remember #bAthena Pierce#k from Henesys who helped you make the 1st and 2nd job advancements?");
