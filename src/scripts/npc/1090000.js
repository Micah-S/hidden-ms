...
/*
	...
	Description: 		Pirate Job Advancement
*/
        ...
        if (status == 0) {
            if (cm.getJob().equals(MapleJob.BEGINNER)) {
                ...
                if (cm.getLevel() >= 30 && cm.getJob().equals(MapleJob.PIRATE)) {
					if (cm.getQuestStatus(2192).equals(Packages.net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED)) {
						status = 20;
					...
					} else if (cm.getQuestStatus(2191).equals(Packages.net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED)) {
						status = 30;
					...
					} else if (cm.getQuestStatus(2191).equals(Packages.net.sf.odinms.client.MapleQuestStatus.Status.STARTED)) {
						status = 15;
						cm.sendYesNo("Would you like to take the test for the second job advancement?");
					} else if (cm.getQuestStatus(2192).equals(Packages.net.sf.odinms.client.MapleQuestStatus.Status.STARTED)) {
						status = 10;
						cm.sendYesNo("Would you like to take the test for the second job advancement?");
                    } else {
