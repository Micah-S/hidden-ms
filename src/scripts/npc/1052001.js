	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (cm.getJob().equals(Packages.net.sf.odinms.client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					&& cm.getJob().equals(Packages.net.sf.odinms.client.MapleJob.THIEF)) {
			...
			if (selection == 0) {
				jobName = "Assassin";
				job = Packages.net.sf.odinms.client.MapleJob.ASSASSIN;
			} else {
				jobName = "Bandit";
				job = Packages.net.sf.odinms.client.MapleJob.BANDIT;
			}
